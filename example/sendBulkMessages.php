...
try {
    ...
    $msg = $smsgw->sendMessages($messages, USE_SPECIFIED, [1, "2|0", "2|1"]);
    
    // Send messages on schedule using the primary device.
    $msg = $smsgw->sendMessages($messages, null, null, strtotime("+2 minutes"));
    
    // Send a message to contacts in contacts list with ID of 1.
    $msg = $smsgw->sendMessageToContactsList(1, "Test", USE_SPECIFIED, 1);
    
    // Send a message on schedule to contacts in contacts list with ID of 1.
    $msg = $smsgw->sendMessageToContactsList(1, "Test", null, null, strtotime("+2 minutes"));
    ...
    $msg = $smsgw->sendMessages($mmsMessages, USE_ALL_SIMS);
